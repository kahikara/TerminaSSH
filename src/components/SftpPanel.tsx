...
function getBreadcrumbMenuPosition(
  ...
  const left = Math.max(
    8,
    Math.min(buttonRect.left - panelRect.left, panelRect.width - menuWidth - 8)
  )

  const openUp = panelRect.bottom - buttonRect.bottom < menuHeight + gap
  const top = openUp
    ? Math.max(8, buttonRect.top - panelRect.top - menuHeight - gap)
    : Math.min(panelRect.height - menuHeight - 8, buttonRect.bottom - panelRect.top + gap)
  ...
  async function openEditor(file: FileItem) {
    ...
    try {
      ...
      const win = new WebviewWindow(label, {
        ...
        url: editorUrl,
        width: Boolean(linuxWindowMode?.wayland_undecorated) ? 1180 : editorWindowState.width,
        height: Boolean(linuxWindowMode?.wayland_undecorated) ? 820 : editorWindowState.height,
        minWidth: 760,
        minHeight: 520,
        center: Boolean(linuxWindowMode?.wayland_undecorated) ? true : !editorWindowState.maximized,
        resizable: true,
        visible: true,
        focus: true,
        skipTaskbar: false,
